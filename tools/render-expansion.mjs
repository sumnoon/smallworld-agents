// Native Canvas visual verification of the actual expanded-map renderer.
...
const root=path.resolve(import.meta.dirname,'..'),state=JSON.parse(fs.readFileSync(path.join(root,'.video-build/expansion-state.json'),'utf8'));
...
save('expanded-town');
for(const place of ['market','garden','plaza','waterfront']){renderer.focusPlace(place);renderer.draw();save('expanded-'+place);}
console.log('Rendered the full expanded map and all four new districts.');
