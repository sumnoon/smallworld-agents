...
Updated 2026-09-10. The playable roadmap includes optional Ollama semantic retrieval, evidence-triggered reflection, ordered task plans and reprioritization, physical invitations and appointments, enterable cutaway rooms and reserved object use, atomic saves and checksummed recorded playback, an editable scenario format, browser voice controls, and a 25-resident option.

**Implemented scope versus research target.** The application retains Canvas/JavaScript, standard-library Python HTTP/SSE, and SQLite. The framework migration proposed below was not necessary for this release. Fifty-seven automated tests pass. The map expansion adds four original sprites, four new destinations, a 24 x 24 terrain layout, district camera controls, and tested preservation of classic saved towns. Offline scale and ablation runs, a live gemma4:31b task-plan check, and live embeddinggemma retrieval are documented in [the acceptance matrix](docs/ROADMAP_IMPLEMENTATION.md) and [evaluation report](docs/evaluation/README.md). Human believability evaluation remains outstanding. Replay renders recorded states rather than rerunning cognition. The larger design below remains a research reference, not a claim of exact experimental reproduction.
